...
import { DeepbookUtils, extractStructTagFromType } from '../utils'
...
export class RouterModule implements IModule {
  ...
  async deepbookPreswap(pool: string, is_bid: boolean, amount_in: number, taker_fee_rate: number, baseAsset: string, quoteAsset: string) {
    ...
    if (is_bid) {
      // base to quote
      const asks = await DeepbookUtils.getPoolAsks(this.sdk, pool, baseAsset, quoteAsset)!
      if (asks!.length === 0) {
        is_exceed = true
        amount_out = 0
      } else {
      ...
      // quote to base
      const bids = await DeepbookUtils.getPoolBids(this.sdk, pool, baseAsset, quoteAsset)!
      if (bids!.length === 0) {
        is_exceed = true
        amount_out = 0
      } else {
