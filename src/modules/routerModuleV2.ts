...
export class RouterModuleV2 implements IModule {
  ...
  async getBestRouter(
    ...
    let version = 'v2'
    const apiUrl = `${
      this.sdk.sdkOptions.aggregatorUrl
    }?from=${from}&to=${to}&amount=${amount.toString()}&by_amount_in=${byAmountIn}&order_split=${orderSplit}&external_router=${externalRouter}`
    ...
    if (result?.isTimeout) {
      ...
      for (const path of priceResult.paths) {
        ...
        if (path.poolAddress.length > 1) {
          ...
          const path1: BasePath = {
            ...
            inputAmount: priceResult.byAmountIn ? path.rawAmountLimit[1].toString() : path.rawAmountLimit[0].toString(),
      ...
      const aggregatorResult: AggregatorResult = {
        ...
        isTimeout: true,
        inputAmount: priceResult.amountIn,
        outputAmount: priceResult.amountOut,
