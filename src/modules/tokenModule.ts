    ...
    while (true) {
      ...
      tx.moveCall({
        ...
        arguments: isOwnerRequest
          ? [tx.object(tokenConfig.coin_registry_id), tx.pure.address(listOwnerAddr), tx.pure.u64(index), tx.pure.u64(limit)]
          : [tx.object(tokenConfig.coin_registry_id), tx.pure.u64(index), tx.pure.u64(limit)],
      })
  ...
  private async factchPoolList(listOwnerAddr = '', forceRefresh = false): Promise<PoolInfo[]> {
    ...
    while (true) {
      ...
      tx.moveCall({
        ...
        arguments: isOwnerRequest
          ? [tx.object(tokenConfig.pool_registry_id), tx.pure.address(listOwnerAddr), tx.pure.u64(index), tx.pure.u64(limit)]
          : [tx.object(tokenConfig.pool_registry_id), tx.pure.u64(index), tx.pure.u64(limit)],
      })
